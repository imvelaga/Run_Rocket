...
cursor = conn.cursor()
#cursor.execute("SELECT x.ticker_name FROM track_it.stock_master x where serial_no > 156194 and  serial_no < %s and is_available is false order by serial_no",((serial_to,)))
...
for row in rows:
        try:
            ...
            print(ex)
            #send_email("Error: {}".format(str(ex))+ "From callStockDetailsAPI_2")
cursor.close()
conn.close()